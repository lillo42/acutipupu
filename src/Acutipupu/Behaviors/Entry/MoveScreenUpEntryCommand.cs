...
/// </summary>
public class MoveScreenUpEntryCommand : EntryCommand
{
    private readonly ILastTextColumn _lastTextColumn;

    /// <summary>
    /// Initializes a new instance of the <see cref="MoveScreenUpEntryCommand"/> class.
    /// </summary>
    ...
    public MoveScreenUpEntryCommand(IComponentAccessor accessor,
    ...
    protected override void OnExecute(ImmutableList<KeyMessage> keys, Components.Entry entry)
    {
        ...
        var screenArea = entry.ScreenArea with { Y = Math.Max(entry.ScreenArea.Y - number, 0) };
