...
/// </summary>
public class MoveCursorDownEntryCommand : EntryCommand
{
    /// <summary>
    /// Initializes a new instance of the <see cref="MoveCursorDownEntryCommand"/> class.
    /// </summary>
    /// <param name="accessor">The <see cref="IComponentAccessor"/>.</param>
    /// <param name="options">The system options.</param>
    public MoveCursorDownEntryCommand(IComponentAccessor accessor, IOptionsMonitor<AcutipupuAppOptions> options)
    ...
    /// <inheritdoc/>
    protected override KeyBindingCollection GetKey(EntryKeyMap map) => map.Down;
    ...
    protected override void OnExecute(ImmutableList<KeyMessage> keys, Components.Entry entry)
    {
        ...
        var row = Math.Min(indexes.Count - 1, entry.CursorPosition.Row + number);
