...
/// <summary>
/// Move screen down and keeping the cursor position unless it's out of screen area + margin.
/// </summary>
public class MoveScreenDownEntryCommand : EntryCommand
{
    private readonly ILastTextColumn _lastTextColumn;

    /// <summary>
    /// Initializes a new instance of the <see cref="MoveScreenDownEntryCommand"/> class.
    /// </summary>
    ...
    public MoveScreenDownEntryCommand(IComponentAccessor accessor,
    ...
    protected override void OnExecute(ImmutableList<KeyMessage> keys, Components.Entry entry)
    {
        ...
        var screenArea = entry.ScreenArea with { Y = Math.Min(entry.ScreenArea.Y + number, indexes.Count - 1) };
